  ...
  pkgs <- Sys.glob(file.path("/home/akhila/fuzzer_packages/fuzzedchangedpackages","*"))
  #pkgs <- y
for(pkg.i in pkgs){
  exelist <- list()
  unexelist <- list()
...
y <- paste0("/home/akhila/fuzzer_packages/fuzzedchangedpackages/",names(pkg.execlist))
#y <- paste0("~/fuzzer_packages/fuzzedpackages/",names(pkg.execlist))
length(y)
          crash.list <- list()
          for(pks in y){
            pk <- pkg.execlist[[basename(pks)]]
            pkg.path <- file.path("/home/akhila/fuzzer_packages/fuzzedchangedpackages",basename(pks))
    print(pkg.path)
            ...
            for(j in pk){
              ...
              if(fun_name %in% getNamespaceExports_fun(paste0("/home/akhila/fuzzer_packages/fuzzedchangedpackages/",basename(pks)))){
                symbol <-"::"
                exported = TRUE
              }else{
                symbol <-":::"
