// AUTOMATICALLY GENERATED BY RCPPDEEPSTATE PLEASE DO NOT EDIT BY HAND, INSTEAD EDIT
// hclustMergeOrder_DeepState_TestHarness_generation.cpp and hclustMergeOrder_DeepState_TestHarness_checks.cpp

#include <fstream>
#include <RInside.h>
#include <iostream>
#include <RcppDeepState.h>
#include <qs.h>
#include <DeepState.hpp>

List hclustMergeOrder(NumericMatrix mst, IntegerVector o);

TEST(dbscan_deepstate_test,hclustMergeOrder_test){
  static int rinside_flag = 0;
  if(rinside_flag == 0)
  {
    rinside_flag = 1;
    RInside R;
  } std::time_t current_timestamp = std::time(0);
  std::cout << "input starts" << std::endl;
  NumericMatrix mst  = RcppDeepState_NumericMatrix();
  std::string mst_t = "/home/akhila/fuzzer_packages/fuzzedpackages/dbscan/inst/testfiles/hclustMergeOrder/libFuzzer_hclustMergeOrder/libfuzzer_inputs/" + std::to_string(current_timestamp) +
          "_mst.qs";
  qs::c_qsave(mst,mst_t,
		"high", "zstd", 1, 15, true, 1);
  std::cout << "mst values: "<< mst << std::endl;
  IntegerVector o  = RcppDeepState_IntegerVector();
  std::string o_t = "/home/akhila/fuzzer_packages/fuzzedpackages/dbscan/inst/testfiles/hclustMergeOrder/libFuzzer_hclustMergeOrder/libfuzzer_inputs/" + std::to_string(current_timestamp) +
          "_o.qs";
  qs::c_qsave(o,o_t,
		"high", "zstd", 1, 15, true, 1);
  std::cout << "o values: "<< o << std::endl;
  std::cout << "input ends" << std::endl;
  try{
    hclustMergeOrder(mst,o);
  }
  catch(Rcpp::exception& e){
    std::cout<<"Exception Handled"<<std::endl;
  }
}
