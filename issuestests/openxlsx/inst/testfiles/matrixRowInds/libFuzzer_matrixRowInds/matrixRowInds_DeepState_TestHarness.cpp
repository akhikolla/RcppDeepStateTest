// AUTOMATICALLY GENERATED BY RCPPDEEPSTATE PLEASE DO NOT EDIT BY HAND, INSTEAD EDIT
// matrixRowInds_DeepState_TestHarness_generation.cpp and matrixRowInds_DeepState_TestHarness_checks.cpp

#include <fstream>
#include <RInside.h>
#include <iostream>
#include <RcppDeepState.h>
#include <qs.h>
#include <DeepState.hpp>

IntegerVector matrixRowInds(IntegerVector indices);

TEST(openxlsx_deepstate_test,matrixRowInds_test){
  static int rinside_flag = 0;
  if(rinside_flag == 0)
  {
    rinside_flag = 1;
    RInside R;
  } std::time_t current_timestamp = std::time(0);
  std::cout << "input starts" << std::endl;
  IntegerVector indices  = RcppDeepState_IntegerVector();
  std::string indices_t = "/home/akhila/fuzzer_packages/fuzzedpackages/openxlsx/inst/testfiles/matrixRowInds/libFuzzer_matrixRowInds/libfuzzer_inputs/" + std::to_string(current_timestamp) +
          "_indices.qs";
  qs::c_qsave(indices,indices_t,
		"high", "zstd", 1, 15, true, 1);
  std::cout << "indices values: "<< indices << std::endl;
  std::cout << "input ends" << std::endl;
  try{
    matrixRowInds(indices);
  }
  catch(Rcpp::exception& e){
    std::cout<<"Exception Handled"<<std::endl;
  }
}
