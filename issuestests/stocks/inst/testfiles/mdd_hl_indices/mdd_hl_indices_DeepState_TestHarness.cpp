#include <fstream>
#include <RInside.h>
#include <iostream>
#include <RcppDeepState.h>
#include <qs.h>
#include <DeepState.hpp>

NumericVector mdd_hl_indices(NumericVector highs, NumericVector lows);

TEST(stocks_deepstate_test,mdd_hl_indices_test){
  RInside R;
  std::cout << "input starts" << std::endl;
  NumericVector highs  = RcppDeepState_NumericVector();
  qs::c_qsave(highs,"/home/akhila/R/x86_64-pc-linux-gnu-library/3.6/RcppDeepState/extdata/issuestests/stocks/inst/testfiles/mdd_hl_indices/inputs/highs.qs",
		"high", "zstd", 1, 15, true, 1);
  std::cout << "highs values: "<< highs << std::endl;
  NumericVector lows  = RcppDeepState_NumericVector();
  qs::c_qsave(lows,"/home/akhila/R/x86_64-pc-linux-gnu-library/3.6/RcppDeepState/extdata/issuestests/stocks/inst/testfiles/mdd_hl_indices/inputs/lows.qs",
		"high", "zstd", 1, 15, true, 1);
  std::cout << "lows values: "<< lows << std::endl;
  std::cout << "input ends" << std::endl;
  try{
    mdd_hl_indices(highs,lows);
  }
  catch(Rcpp::exception& e){
    std::cout<<"Exception Handled"<<std::endl;
  }
}
